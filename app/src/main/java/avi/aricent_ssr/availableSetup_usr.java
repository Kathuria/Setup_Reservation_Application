...
public class availableSetup_usr extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.availablesetup_usr);
		
		  db = new MainDatabaseHandler(this);
		  tvas= (TextView) findViewById(R.id.tvasU);
		  listView = (ListView) findViewById(R.id.availSetupListViewU);
		//method for getting free setup list from database
	      ...
	      listView.setOnItemClickListener(new OnItemClickListener() {
	 	  ...
	 	  public void onItemClick(AdapterView<?> listView, View view, int position, long id) {
			...
			// set dialog message
			alertDialogBuilder.setMessage(log).setCancelable(false).setPositiveButton("Book",new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,int id) {
								    Intent intent = new Intent(context,book_setup_usr.class);
								    intent.putExtra("setup_name", setup_name);
								    startActivity(intent);
								   availableSetup_usr.this.finish();
							}
						  })
